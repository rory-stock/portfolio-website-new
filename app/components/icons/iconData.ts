export const iconNames = [
  "settings",
  "dashboard",
  "overview",
  "journal",
  "globe",
  "logout",
  "back",
  "smile",
  "footer",
  "cross",
  "cleanup",
  "file",
  "event",
  "chevron",
  "star",
  "check",
  "info",
  "bin",
  "refresh",
  "openEye",
  "edit",
  "download",
  "lock",
  "moreOptions",
  "share",
] as const;

export type IconName = (typeof iconNames)[number];

export type StrokeLinecap = "round" | "butt" | "square" | "inherit";
export type StrokeLinejoin = "round" | "inherit" | "bevel" | "miter";
export type FillRule = "nonzero" | "evenodd" | "inherit";
export type ClipRule = "nonzero" | "evenodd" | "inherit";

interface ElementStyle {
  fill?: string;
  stroke?: string;
  strokeWidth?: number | string;
  opacity?: number | string;

  fillOpacity?: string | number;
  strokeOpacity?: string | number;

  strokeLinecap?: StrokeLinecap;
  strokeLinejoin?: StrokeLinejoin;
  strokeMiterlimit?: number | string;

  clipRule?: ClipRule;
  fillRule?: FillRule;
}

export type IconElement =
  | ({ type: "path"; d: string } & ElementStyle)
  | ({
      type: "circle";
      cx: number | string;
      cy: number | string;
      r: number | string;
    } & ElementStyle)
  | ({
      type: "line";
      x1: number;
      y1: number;
      x2: number;
      y2: number;
    } & ElementStyle);

export interface IconData {
  viewBox: string;

  fill?: string;
  stroke?: string;
  strokeWidth?: number | string;
  opacity?: number | string;

  fillOpacity?: number | string;
  strokeOpacity?: number | string;

  strokeLinecap?: StrokeLinecap;
  strokeLinejoin?: StrokeLinejoin;
  strokeMiterlimit?: number | string;

  clipRule?: ClipRule;
  fillRule?: FillRule;

  elements: IconElement[];
}

export const iconData: Record<IconName, IconData> = {
  settings: {
    viewBox: "0 0 24 24",
    elements: [
      {
        type: "path",
        d: "M10.825 22q-.675 0-1.162-.45t-.588-1.1L8.85 18.8q-.325-.125-.612-.3t-.563-.375l-1.55.65q-.625.275-1.25.05t-.975-.8l-1.175-2.05q-.35-.575-.2-1.225t.675-1.075l1.325-1Q4.5 12.5 4.5 12.337v-.675q0-.162.025-.337l-1.325-1Q2.675 9.9 2.525 9.25t.2-1.225L3.9 5.975q.35-.575.975-.8t1.25.05l1.55.65q.275-.2.575-.375t.6-.3l.225-1.65q.1-.65.588-1.1T10.825 2h2.35q.675 0 1.163.45t.587 1.1l.225 1.65q.325.125.613.3t.562.375l1.55-.65q.625-.275 1.25-.05t.975.8l1.175 2.05q.35.575.2 1.225t-.675 1.075l-1.325 1q.025.175.025.338v.674q0 .163-.05.338l1.325 1q.525.425.675 1.075t-.2 1.225l-1.2 2.05q-.35.575-.975.8t-1.25-.05l-1.5-.65q-.275.2-.575.375t-.6.3l-.225 1.65q-.1.65-.587 1.1t-1.163.45zM11 20h1.975l.35-2.65q.775-.2 1.438-.587t1.212-.938l2.475 1.025l.975-1.7l-2.15-1.625q.125-.35.175-.737T17.5 12t-.05-.787t-.175-.738l2.15-1.625l-.975-1.7l-2.475 1.05q-.55-.575-1.212-.962t-1.438-.588L13 4h-1.975l-.35 2.65q-.775.2-1.437.588t-1.213.937L5.55 7.15l-.975 1.7l2.15 1.6q-.125.375-.175.75t-.05.8q0 .4.05.775t.175.75l-2.15 1.625l.975 1.7l2.475-1.05q.55.575 1.213.963t1.437.587zm1.05-4.5q1.45 0 2.475-1.025T15.55 12t-1.025-2.475T12.05 8.5q-1.475 0-2.487 1.025T8.55 12t1.013 2.475T12.05 15.5M12 12",
      },
    ],
  },
  dashboard: {
    viewBox: "0 0 15 15",
    elements: [
      {
        type: "path",
        d: "M5.25 8c.228 0 .426 0 .6.042a1.5 1.5 0 0 1 1.11 1.107c.04.175.04.373.04.601v2.5c0 .228 0 .426-.041.6a1.5 1.5 0 0 1-1.109 1.11c-.174.04-.372.04-.6.04h-2.5c-.228 0-.426 0-.6-.041a1.5 1.5 0 0 1-1.108-1.108C1 12.676 1 12.478 1 12.25v-2.5c0-.228 0-.426.042-.6a1.5 1.5 0 0 1 1.107-1.108C2.324 8 2.522 8 2.75 8zm7 0c.228 0 .426 0 .6.042a1.5 1.5 0 0 1 1.109 1.107c.042.175.041.373.041.601v2.5c0 .228 0 .426-.041.6a1.5 1.5 0 0 1-1.108 1.109c-.175.042-.373.041-.601.041h-2.5c-.228 0-.426 0-.6-.041a1.5 1.5 0 0 1-1.108-1.108C8 12.676 8 12.478 8 12.25v-2.5c0-.228 0-.426.042-.6a1.5 1.5 0 0 1 1.107-1.108C9.324 8 9.522 8 9.75 8zM2.8 9c-.307 0-.373.003-.416.014a.5.5 0 0 0-.37.37c-.01.043-.014.11-.014.416v2.4c0 .308.003.374.014.417a.5.5 0 0 0 .37.37c.043.01.11.013.416.013h2.4c.308 0 .374-.003.417-.014a.5.5 0 0 0 .37-.369c.01-.043.013-.11.013-.417V9.8c0-.307-.003-.373-.014-.416a.5.5 0 0 0-.369-.37C5.574 9.004 5.507 9 5.2 9zm7 0c-.307 0-.373.003-.416.014a.5.5 0 0 0-.37.37c-.01.043-.014.11-.014.416v2.4c0 .308.003.374.014.417a.5.5 0 0 0 .37.37c.043.01.11.013.416.013h2.4c.308 0 .374-.003.417-.014a.5.5 0 0 0 .37-.369c.01-.043.013-.11.013-.417V9.8c0-.307-.004-.373-.014-.416a.5.5 0 0 0-.369-.37c-.043-.01-.11-.014-.417-.014zM5.25 1c.228 0 .426 0 .6.042a1.5 1.5 0 0 1 1.11 1.107c.04.175.04.373.04.601v2.5c0 .228 0 .426-.041.6A1.5 1.5 0 0 1 5.85 6.96c-.174.04-.372.04-.6.04h-2.5c-.228 0-.426 0-.6-.041A1.5 1.5 0 0 1 1.041 5.85C1 5.676 1 5.478 1 5.25v-2.5c0-.228 0-.426.042-.6a1.5 1.5 0 0 1 1.107-1.108C2.324 1 2.522 1 2.75 1zm7 0c.228 0 .426 0 .6.042a1.5 1.5 0 0 1 1.109 1.107c.042.175.041.373.041.601v2.5c0 .228 0 .426-.041.6a1.5 1.5 0 0 1-1.109 1.11c-.174.04-.372.04-.6.04h-2.5c-.228 0-.426 0-.6-.041A1.5 1.5 0 0 1 8.041 5.85C8 5.676 8 5.478 8 5.25v-2.5c0-.228 0-.426.042-.6a1.5 1.5 0 0 1 1.107-1.108C9.324 1 9.522 1 9.75 1zM2.8 2c-.307 0-.373.003-.416.014a.5.5 0 0 0-.37.37c-.01.043-.014.11-.014.416v2.4c0 .308.003.374.014.417a.5.5 0 0 0 .37.37c.043.01.11.013.416.013h2.4c.308 0 .374-.003.417-.014a.5.5 0 0 0 .37-.369c.01-.043.013-.11.013-.417V2.8c0-.307-.003-.373-.014-.416a.5.5 0 0 0-.369-.37C5.574 2.004 5.507 2 5.2 2zm7 0c-.307 0-.373.003-.416.014a.5.5 0 0 0-.37.37c-.01.043-.014.11-.014.416v2.4c0 .308.003.374.014.417a.5.5 0 0 0 .37.37c.043.01.11.013.416.013h2.4c.308 0 .374-.003.417-.014a.5.5 0 0 0 .37-.369c.01-.043.013-.11.013-.417V2.8c0-.307-.004-.373-.014-.416a.5.5 0 0 0-.369-.37c-.043-.01-.11-.014-.417-.014z",
      },
    ],
  },
  overview: {
    viewBox: "0 0 15 15",
    elements: [
      {
        type: "path",
        d: "M7.173.147a.6.6 0 0 1 .748.075l6.75 6.64l.077.093a.6.6 0 0 1-.824.838l-.095-.076l-.83-.816v5.6a.5.5 0 0 1-.5.5h-10a.5.5 0 0 1-.5-.5V6.9l-.828.816a.6.6 0 0 1-.842-.855l6.75-6.64zM3 5.917V12h3V8.5l.01-.1A.5.5 0 0 1 6.5 8h3l.1.01a.5.5 0 0 1 .4.49V12h2V5.917L7.5 1.491zM7 12h2V9H7z",
      },
    ],
  },
  journal: {
    viewBox: "0 0 15 15",
    elements: [
      {
        type: "path",
        d: "M9 0a1 1 0 0 1 1 1v1h4a1 1 0 0 1 1 1v3a3 3 0 0 1-1 2.234V11.5a1.5 1.5 0 0 1-1.5 1.5h-10A1.5 1.5 0 0 1 1 11.5V8.233A3 3 0 0 1 0 6V3a1 1 0 0 1 1-1h4V1a1 1 0 0 1 1-1zm4 8.827c-.313.11-.65.173-1 .173H8v.5a.5.5 0 0 1-1 0V9H3c-.35 0-.687-.062-1-.173V11.5a.5.5 0 0 0 .5.5h10a.5.5 0 0 0 .5-.5zM1 6c0 .654.313 1.235.8 1.6l.13.09c.31.196.676.31 1.07.31h4v-.5a.5.5 0 0 1 1 0V8h4c.45 0 .866-.148 1.2-.4l.174-.147C13.76 7.088 14 6.573 14 6V3H1zm5-4h3V1H6z",
      },
    ],
  },
  globe: {
    viewBox: "0 0 15 15",
    elements: [
      {
        type: "path",
        d: "M7.5.9a6.6 6.6 0 1 1-.002 13.201A6.6 6.6 0 0 1 7.5.901m-2.216 9.805a7 7 0 0 0 1.575 2.458q.12.013.24.02v-2.338a17 17 0 0 1-1.815-.14m4.489-.006c-.611.083-1.242.131-1.873.145v2.34q.122-.008.242-.021A7 7 0 0 0 9.773 10.7m-7.415-.737a5.72 5.72 0 0 0 3.355 2.95a8.2 8.2 0 0 1-1.217-2.333a11 11 0 0 1-2.138-.617m10.283 0a11 11 0 0 1-2.072.605a8 8 0 0 1-1.253 2.335a5.7 5.7 0 0 0 3.325-2.94M4.71 7.9c.03.703.139 1.398.327 2.063c.661.104 1.358.165 2.063.182V7.9zm3.19 2.245a16 16 0 0 0 2.13-.192a8.8 8.8 0 0 0 .337-2.053H7.9zM1.816 7.9q.041.579.191 1.12c.628.345 1.407.612 2.27.8a10 10 0 0 1-.268-1.92zm9.252 0a9.5 9.5 0 0 1-.274 1.905c.735-.164 1.408-.386 1.975-.666l.224-.118q.15-.541.192-1.121zm-6.74-2.925c-.854.183-1.625.443-2.253.777q-.208.645-.258 1.347h2.192a9.8 9.8 0 0 1 .319-2.124M7.1 4.662a16 16 0 0 0-2.003.173a9 9 0 0 0-.388 2.264H7.1zm.8 2.437h2.467a8.7 8.7 0 0 0-.399-2.255A16 16 0 0 0 7.9 4.662zm2.84-2.11c.188.686.298 1.397.328 2.11h2.117a5.7 5.7 0 0 0-.26-1.348l-.156-.082c-.58-.287-1.272-.514-2.028-.68M5.714 2.087a5.72 5.72 0 0 0-3.231 2.707a11 11 0 0 1 2.086-.58a8 8 0 0 1 1.145-2.127m3.603.01c.5.642.892 1.362 1.178 2.128c.724.136 1.41.325 2.021.568a5.7 5.7 0 0 0-3.2-2.696M7.9 3.963a17 17 0 0 1 1.788.135a7 7 0 0 0-1.545-2.261a6 6 0 0 0-.243-.022zm-.8-2.148q-.122.009-.242.022A7 7 0 0 0 5.367 4.09c.567-.072 1.15-.115 1.732-.128z",
      },
    ],
  },
  logout: {
    viewBox: "0 0 24 24",
    fill: "none",
    stroke: "currentColor",
    strokeLinecap: "round",
    strokeLinejoin: "round",
    strokeWidth: "2",
    elements: [
      {
        type: "path",
        d: "M14 8V6a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h7a2 2 0 0 0 2-2v-2",
      },
      {
        type: "path",
        d: "M9 12h12l-3-3m0 6l3-3",
      },
    ],
  },
  back: {
    viewBox: "0 0 24 24",
    fill: "none",
    stroke: "currentColor",
    strokeLinecap: "square",
    strokeWidth: "2",
    elements: [
      {
        type: "path",
        d: "m13 17.5l5.5-5.5L13 6.5m4.25 5.5h-13",
      },
    ],
  },
  smile: {
    viewBox: "0 0 496 512",
    elements: [
      {
        type: "path",
        d: "M248 8C111 8 0 119 0 256s111 248 248 248s248-111 248-248S385 8 248 8m0 448c-110.3 0-200-89.7-200-200S137.7 56 248 56s200 89.7 200 200s-89.7 200-200 200m-80-216c17.7 0 32-14.3 32-32s-14.3-32-32-32s-32 14.3-32 32s14.3 32 32 32m160 0c17.7 0 32-14.3 32-32s-14.3-32-32-32s-32 14.3-32 32s14.3 32 32 32m4 72.6c-20.8 25-51.5 39.4-84 39.4s-63.2-14.3-84-39.4c-8.5-10.2-23.7-11.5-33.8-3.1c-10.2 8.5-11.5 23.6-3.1 33.8c30 36 74.1 56.6 120.9 56.6s90.9-20.6 120.9-56.6c8.5-10.2 7.1-25.3-3.1-33.8c-10.1-8.4-25.3-7.1-33.8 3.1",
      },
    ],
  },
  footer: {
    viewBox: "0 0 16 16",
    elements: [
      {
        type: "path",
        d: "M6 11a1 1 0 1 0 0 2h4a1 1 0 1 0 0-2zM4.75 1A1.75 1.75 0 0 0 3 2.75v10.5c0 .966.784 1.75 1.75 1.75h6.5A1.75 1.75 0 0 0 13 13.25V2.75A1.75 1.75 0 0 0 11.25 1zM4 2.75A.75.75 0 0 1 4.75 2h6.5a.75.75 0 0 1 .75.75v10.5a.75.75 0 0 1-.75.75h-6.5a.75.75 0 0 1-.75-.75z",
      },
    ],
  },
  cross: {
    viewBox: "0 0 15 15",
    elements: [
      {
        type: "path",
        d: "M10.969 3.219a.574.574 0 1 1 .812.812L8.313 7.5l3.468 3.469l.074.09a.575.575 0 0 1-.796.796l-.09-.074L7.5 8.312l-3.469 3.47a.574.574 0 1 1-.812-.813L6.688 7.5l-3.47-3.469l-.073-.09a.575.575 0 0 1 .796-.797l.09.075L7.5 6.687z",
      },
    ],
  },
  cleanup: {
    viewBox: "0 0 24 24",
    fillRule: "evenodd",
    elements: [
      {
        type: "path",
        d: "m12.594 23.258l-.012.002l-.071.035l-.02.004l-.014-.004l-.071-.036q-.016-.004-.024.006l-.004.01l-.017.428l.005.02l.01.013l.104.074l.015.004l.012-.004l.104-.074l.012-.016l.004-.017l-.017-.427q-.004-.016-.016-.018m.264-.113l-.014.002l-.184.093l-.01.01l-.003.011l.018.43l.005.012l.008.008l.201.092q.019.005.029-.008l.004-.014l-.034-.614q-.005-.019-.02-.022m-.715.002a.02.02 0 0 0-.027.006l-.006.014l-.034.614q.001.018.017.024l.015-.002l.201-.093l.01-.008l.003-.011l.018-.43l-.003-.012l-.01-.01z",
      },
      {
        type: "path",
        d: "M17.278 2.613a1 1 0 0 1 1.89.643l-.038.11l-2.61 6.42l.657.175c1.05.281 1.924 1.134 2.09 2.298c.142 1 .275 2.52.092 4.086c-.182 1.552-.69 3.278-1.947 4.546c-.462.466-1.125.54-1.573.548c-.511.008-1.1-.07-1.705-.19c-1.216-.242-2.674-.69-4.054-1.166l-.414-.145l-.813-.294l-.78-.291l-.734-.283l-.978-.388l-.822-.335l-.817-.345a1 1 0 0 1-.228-1.708c1.377-1.08 2.67-2.322 3.761-3.469l.529-.564l.25-.274l.472-.527l.22-.252l.594-.695l.337-.406a3.1 3.1 0 0 1 2.981-1.087l.199.046l.737.197zM10.5 13.348a44 44 0 0 1-3.479 3.444q.863.349 1.733.68a7.3 7.3 0 0 0 1.426-1.338a7 7 0 0 0 .488-.654l.142-.232a1 1 0 0 1 1.747.973c-.234.42-.527.814-.832 1.184a10 10 0 0 1-.792.856c.462.158.924.308 1.372.446c.373-.257.81-.785 1.206-1.385q.239-.36.452-.74l.204-.384a1 1 0 0 1 1.793.887c-.229.462-.496.909-.78 1.339a11 11 0 0 1-.634.868l.421.082c.362.067.744.114 1.089.043c.766-.815 1.163-1.998 1.316-3.305q.053-.456.068-.904zm2.819-2.35a1.09 1.09 0 0 0-1.116.378l-.243.293l5.398 1.446l-.047-.392l-.024-.182c-.037-.253-.216-.491-.511-.61l-.116-.038zM5.565 7.716l.064.14A3.26 3.26 0 0 0 6.866 9.22l.1.058a.068.068 0 0 1 0 .118l-.1.058A3.26 3.26 0 0 0 5.63 10.82l-.064.139a.071.071 0 0 1-.13 0l-.064-.14a3.26 3.26 0 0 0-1.237-1.364l-.1-.058a.068.068 0 0 1 0-.118l.1-.058A3.26 3.26 0 0 0 5.37 7.855l.064-.139a.071.071 0 0 1 .13 0Zm2.832-4.859c.04-.09.166-.09.206 0l.102.222a5.2 5.2 0 0 0 1.97 2.171l.157.093a.108.108 0 0 1 0 .189l-.158.092a5.2 5.2 0 0 0-1.97 2.172l-.1.222a.113.113 0 0 1-.207 0l-.102-.222a5.2 5.2 0 0 0-1.97-2.172l-.158-.092a.108.108 0 0 1 0-.189l.159-.093a5.2 5.2 0 0 0 1.97-2.171l.1-.222Z",
      },
    ],
  },
  file: {
    viewBox: "0 0 24 24",
    fill: "none",
    stroke: "currentColor",
    strokeLinecap: "round",
    strokeLinejoin: "round",
    strokeWidth: "2",
    elements: [
      {
        type: "path",
        d: "M6 22a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h8a2.4 2.4 0 0 1 1.704.706l3.588 3.588A2.4 2.4 0 0 1 20 8v12a2 2 0 0 1-2 2z",
      },
      { type: "path", d: "M14 2v5a1 1 0 0 0 1 1h5M10 9H8m8 4H8m8 4H8" },
    ],
  },
  event: {
    viewBox: "0 0 24 24",
    elements: [
      {
        type: "path",
        d: "M17 12h-5v5h5zM16 1v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19a2 2 0 0 0 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2h-1V1zm3 18H5V8h14z",
      },
    ],
  },
  chevron: {
    viewBox: "0 0 15 15",
    elements: [
      {
        type: "path",
        d: "M11.158 6.135a.5.5 0 0 1 .684.73l-4 3.75a.5.5 0 0 1-.684 0l-4-3.75l-.066-.077a.5.5 0 0 1 .67-.715l.08.062L7.5 9.564z",
      },
    ],
  },
  star: {
    viewBox: "0 0 24 24",
    elements: [
      {
        type: "path",
        d: "M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.62L12 2L9.19 8.62L2 9.24l5.45 4.73L5.82 21z",
      },
    ],
  },
  check: {
    viewBox: "0 0 24 24",
    elements: [
      {
        type: "path",
        d: "M9 16.17L4.83 12l-1.42 1.41L9 19L21 7l-1.41-1.41z",
      },
    ],
  },
  info: {
    viewBox: "0 0 24 24",
    elements: [
      {
        type: "path",
        d: "M11 7h2v2h-2zm0 4h2v6h-2zm1-9C6.48 2 2 6.48 2 12s4.48 10 10 10s10-4.48 10-10S17.52 2 12 2m0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8s8 3.59 8 8s-3.59 8-8 8",
      },
    ],
  },
  bin: {
    viewBox: "0 0 24 24",
    fill: "none",
    stroke: "currentColor",
    strokeLinecap: "round",
    strokeLinejoin: "round",
    strokeWidth: "2",
    elements: [
      {
        type: "path",
        d: "M10 11v6m4-6v6m5-11v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2",
      },
    ],
  },
  refresh: {
    viewBox: "0 0 24 24",
    elements: [
      {
        type: "path",
        d: "M17.65 6.35A7.96 7.96 0 0 0 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08A5.99 5.99 0 0 1 12 18c-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4z",
      },
    ],
  },
  openEye: {
    viewBox: "0 0 24 24",
    elements: [
      {
        type: "path",
        d: "M12 9a3 3 0 0 1 3 3a3 3 0 0 1-3 3a3 3 0 0 1-3-3a3 3 0 0 1 3-3m0-4.5c5 0 9.27 3.11 11 7.5c-1.73 4.39-6 7.5-11 7.5S2.73 16.39 1 12c1.73-4.39 6-7.5 11-7.5M3.18 12a9.821 9.821 0 0 0 17.64 0a9.821 9.821 0 0 0-17.64 0",
      },
    ],
  },
  edit: {
    viewBox: "0 0 24 24",
    fill: "none",
    stroke: "currentColor",
    strokeLinecap: "round",
    strokeLinejoin: "round",
    strokeWidth: "2",
    elements: [
      {
        type: "path",
        d: "M12 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7",
      },
      {
        type: "path",
        d: "M18.375 2.625a1 1 0 0 1 3 3l-9.013 9.014a2 2 0 0 1-.853.505l-2.873.84a.5.5 0 0 1-.62-.62l.84-2.873a2 2 0 0 1 .506-.852z",
      },
    ],
  },
  download: {
    viewBox: "0 0 16 16",
    elements: [
      {
        type: "path",
        d: "M3.5 13h9a.5.5 0 0 1 .09.992L12.5 14h-9a.5.5 0 0 1-.09-.992zh9zM7.91 1.008L8 1a.5.5 0 0 1 .492.41l.008.09v8.792l2.682-2.681a.5.5 0 0 1 .638-.058l.07.058a.5.5 0 0 1 .057.638l-.058.069l-3.535 3.536a.5.5 0 0 1-.638.057l-.07-.057l-3.535-3.536a.5.5 0 0 1 .638-.765l.069.058L7.5 10.292V1.5a.5.5 0 0 1 .41-.492L8 1z",
      },
    ],
  },
  lock: {
    viewBox: "0 0 24 24",
    elements: [
      {
        type: "path",
        d: "M12 17a2 2 0 0 0 2-2a2 2 0 0 0-2-2a2 2 0 0 0-2 2a2 2 0 0 0 2 2m6-9a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V10a2 2 0 0 1 2-2h1V6a5 5 0 0 1 5-5a5 5 0 0 1 5 5v2zm-6-5a3 3 0 0 0-3 3v2h6V6a3 3 0 0 0-3-3",
      },
    ],
  },
  moreOptions: {
    viewBox: "0 0 24 24",
    fill: "none",
    stroke: "currentColor",
    strokeLinecap: "round",
    strokeLinejoin: "round",
    strokeWidth: "2",
    elements: [
      { type: "circle", cx: "12", cy: "12", r: "1" },
      { type: "circle", cx: "12", cy: "5", r: "1" },
      { type: "circle", cx: "12", cy: "19", r: "1" },
    ],
  },
  share: {
    viewBox: "0 0 432 432",
    elements: [
      {
        type: "path",
        d: "m133.74 362.41c-29.628-19.583-48.23-46.205-55.575-80.777-12.032-56.629 18.764-111.82 73.233-130.4 10.592-3.6141 21.534-5.5926 32.798-5.57 21.664 0.043549 43.328 0.008957 64.993-0.001358 6.464-0.003082 6.4797-0.015472 6.4863-6.6883 0.013595-13.665-0.011688-27.33 0.017837-40.995 0.022324-10.336 4.3129-17.115 13.43-20.699 8.878-3.4907 16.629-1.3476 23.285 5.3108 32.399 32.41 64.838 64.781 97.189 97.24 9.6161 9.648 9.5632 22.792-0.10126 32.485-32.123 32.216-64.329 64.35-96.49 96.528-6.9232 6.9268-14.937 10.282-24.419 6.2487-9.3562-3.9802-12.989-11.788-12.919-21.7 0.097611-13.831-0.085388-27.665 0.075241-41.494 0.045228-3.8946-0.83035-5.6738-5.2314-5.6216-17.829 0.21149-35.664-0.090134-53.492 0.11659-29.88 0.3465-53.94 20.107-59.183 49.52-3.0977 17.376 0.56696 33.446 13.602 46.522 5.8953 5.9139 6.7708 12.812 2.8643 18.482-4.4973 6.5282-11.178 7.0831-20.561 1.4957m116.76-196.06c-23.814 0.42044-47.66-1.0648-71.445 0.80501-29.229 2.2979-51.958 16.276-67.639 40.652-19.764 30.725-18.299 62.938-3.9818 95.344 2.1534 4.874 4.7053 9.6476 9.4798 14.432-1.2191-4.7969-1.3478-8.4499-1.1891-12.087 0.1665-3.8149 0.45503-7.6434 1.0062-11.42 5.542-37.97 36.409-66.499 74.337-68.191 23.284-1.0388 46.652-0.25174 69.982-0.18288 8.2871 0.02446 14.095 5.2122 15.341 13.462 0.37155 2.459 0.57999 4.9661 0.58987 7.4523 0.053742 13.498-0.013855 26.997 0.061157 40.494 0.010315 1.8582-0.67102 3.9565 1.1105 5.8028 0.94836-0.82102 1.8629-1.5183 2.669-2.3242 30.404-30.395 60.75-60.849 91.262-91.135 3.4214-3.396 1.7451-5.1178-0.74121-7.5969-29.856-29.769-59.652-59.597-89.471-89.403-1.3414-1.3408-2.7469-2.6176-4.885-4.6484 0 12.218 0.083832 23.186-0.029907 34.152-0.074158 7.1514 0.64322 14.333-0.58615 21.451-1.3137 7.6073-7.1564 12.695-14.876 12.903-3.3307 0.089706-6.6654 0.030059-10.995 0.03836z",
      },
    ],
  },
};
